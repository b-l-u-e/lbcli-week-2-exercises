# Calculate total output value of this transaction in satoshis
raw_tx=""

# Decode the transaction
decoded=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx")

# echo "$decoded"

# Sum up all output values (in BTC)
total_btc=$(echo "$decoded" | jq '[.vout[].value] | add')

# echo "$total_btc"

# Convert BTC to satoshis (multiply by 100,000,000) and format as an integer
total_sat=$(echo "scale=0; $total_btc * 100000000 / 1" | bc)

# echo "$total_sat"

# Output only the total output value in satoshis
echo "$total_sat"
